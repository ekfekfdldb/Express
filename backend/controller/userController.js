...
exports.getRewardTokenBalance = async (request, response, next) => { // 지갑 주소로부터 CTK 토큰 잔액을 조회하는 API 핸들러
    const { address } = request.query; // 쿼리 문자열에서 address 파라미터 추출

    if (!address) { // address가 없을 경우 400 에러 반환
        return response.status(STATUS_CODE.BAD_REQUEST).json({
            message: STATUS_MESSAGE.INVALID_WALLET_ADDRESS, // 지갑 주소가 유효하지 않음을 알림
            data: null
        });
    }

    try {
        const balance = await getRewardTokenBalance(address); // address에 대한 토큰 잔액 조회

        return response.status(STATUS_CODE.OK).json({
            message: STATUS_MESSAGE.GET_BALANCE_SUCCESS, // 잔액 조회 성공 메시지
            data: {
                address, // 요청된 지갑 주소
                balance, // 해당 지갑의 CTK 잔액
                symbol: 'CTK' // 토큰 심볼
            }
        });
    } catch (error) { // 예외 발생 시
